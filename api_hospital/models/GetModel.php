...
class GetModel {
    ...
    static public function getRelData($rel,$key,$select,$orderBy,$orderMode,$startAt,$limit,$distinct) {
        ...
        $bd = new Bd();
        $sql = "select $distinct $select from `$relArray[0]`";
    ...
    static public function getRelDataFilter($rel,$key,$select,$orderBy,$orderMode,$startAt,$limit,$linkTo,$equalTo,$distinct) {
        ...
        foreach($linkToArray as $key=>$value){
            if($key==0) {
                $sql = $sql."$relArray[0].$value= ?";
            } else {
                $sql = $sql." and $relArray[0].$value= ?";
            }
        }
        // comprobamos los parametros que se hayan pasado y los iremos añadiendo a la consulta
        if($orderBy!=null && $orderMode!=null){
    ...
    static public function getDataFilter($table,$select,$linkTo,$equalTo,$orderBy,$orderMode,$startAt,$limit,$distinct) {
        ...
        try {
            ...
            $bd = new Bd();
            $sql = "select $distinct $select from `$table` where ";
            // comprobamos que el array de campos que se pasan para filtrar y el de condiciones tienen la misma longitud
            if(count($linkToArray)!=count($equalToArray)){
                return [];
            }
    ...
    static public function nextCita($id,$date) {
        $bd = new Bd();
        $sql = 'SELECT  paciente.*, cita.*, departamento.*, medico.consulta FROM `cita` inner JOIN medico on cita.id_medico = medico.id_medico inner join departamento on departamento.id_departamento = medico.id_departamento inner join paciente on cita.dni_paciente = paciente.dni_paciente where `fecha` = ? and medico.id_usuario = ? and `estado` = "pendiente" limit 1';
        return $bd->select($sql,[$date,$id])[0];
    }
    static public function getCita($id) {
        $bd = new Bd();
        $sql = 'SELECT paciente.*, cita.*, departamento.*, medico.consulta FROM `cita` inner JOIN medico on cita.id_medico = medico.id_medico inner join departamento on departamento.id_departamento = medico.id_departamento inner join paciente on cita.dni_paciente = paciente.dni_paciente where id_cita = ?';
        return $bd->select($sql,[$id])[0];
    }
    static public function getMail($mail,$filter="email") {
        $bd = new Bd();
        $sql = "select user.id_usuario, user.email, usuario.username from (SELECT email, id_usuario from paciente where `$filter` = ? UNION SELECT email, id_usuario from medico where `$filter` = ?) as user INNER JOIN usuario on usuario.id_usuario = user.id_usuario;";
        return $bd->select($sql,[$mail,$mail]);
    }
    static public function getHistory($id) {
        $bd = new Bd();
        $sql = "SELECT * FROM `historial` INNER JOIN paciente ON paciente.dni_paciente = historial.dni_paciente WHERE paciente.id_usuario = ?";
        return $bd->select($sql,[$id]);
    }
}